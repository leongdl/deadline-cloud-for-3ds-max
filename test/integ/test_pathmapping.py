...
def update_path_mapping_for_checkout(
    ...
    for rule in path_mapping.get("path_mapping_rules", []):
        ...
        dest_path = rule.get("destination_path", "")

        # Find the relative path starting from test/integ/test_scripts
        relative_marker = "test/integ/test_scripts"
        if relative_marker in dest_path.replace("\\", "/"):
            # Extract everything from test/integ/test_scripts onwards
            idx = dest_path.replace("\\", "/").find(relative_marker)
            relative_path = dest_path.replace("\\", "/")[idx:]
            # Construct new destination path using current checkout root
...
def verify_path_mapping_in_log(
    ...
    Returns:
        Tuple of (success, list of matching log lines).
    """
    ...
    return found_valid_remap, matching_lines
...
class TestPathMappingAdaptors:
    ...
    def test_vrmesh_path_remap_adaptor(
        ...
        # Get the checkout root (workspace root)
        checkout_root = Path(__file__).parent.parent.parent
        ...
        if max_log_path and max_log_path.exists():
            ...
            remap_success, remap_lines = verify_path_mapping_in_log(
                max_log_path,
            ...
            assert remap_success, (
                f"Path mapping verification failed. Expected remapping from 'assets/Bench.vrmesh' "
                f"to 'vray_vrmesh_remap_test/scene/Bench.vrmesh'.\n"
                f"Log entries found: {remap_lines}"
            )
            ...
            print(
                f"Warning: Log file not found at {max_log_path or 'unknown path'}, "
