...
└── max_shared/            # Shared utilities (NEW)
    └── utilities/
...
- `get_render_elements()` - Comprehensive render element detection with V-Ray VFB support
...
1. **DRY Principle** - No code duplication between submitter and adaptor
2. **Consistency** - Same behavior for render elements detection in GUI and render execution
3. **Maintainability** - Single source of truth for pymxs render elements logic
4. **Testing** - Shared functions can be tested once and used everywhere
...
The shared `get_render_elements()` function provides comprehensive detection matching Deadline 10:
